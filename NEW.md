...
The main goal is to extract a core features into the system 40`ANTS-DOC`
...
like `IRONCLAD`, 3`BMD` or SWANK should not be required.
...
* Core system `40`ANTS-DOC`` now has only two dependencies on `NAMED-READTABLES`
  and `PYTHONIC-STRING-READER`. If you want to compile a documentation, load
  `40`ANTS-DOC`/`FULL`` system which will download such dependencies as markdown
  parser and more.
...
`
   `WARNING`: Unable to find symbol "`API`" mentioned in (`CL-INFO:@INDEX` [`SECTION`](#x-2840ANTS-DOC-2FLOCATIVES-3ASECTION-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29))
`

I'm planning to extend this fork even more. Read [`TODO`](#x-2840ANTS-DOC-2FDOC-3A-40TODO-2040ANTS-DOC-2FLOCATIVES-3ASECTION-29) section to learn about
...
40`ANTS-DOC` provides an extremely poor man's Explorable Programming
...
fanciness in [`Emacs Integration`](#x-2840ANTS-DOC-2FDOC-3A-40EMACS-INTEGRATION-2040ANTS-DOC-2FLOCATIVES-3ASECTION-29). Generating documentation
...
In typical use, using 40`ANTS-DOC`, packages have no :`EXPORT`'s defined.
...
table of contents, etc is possible by calling the `40`ANTS-DOC`/`DOCUMENT::DOCUMENT``
function.

One can even generate documentation for different, but related
libraries at the same time with the output going to different files,
but with cross-page links being automatically added for symbols
mentioned in docstrings. See `40`ANTS-DOC`/`BUILDER::@GENERATING-DOCUMENTATION`` for
...
definitions tied to symbols. See [`Locative Types`](#x-2840ANTS-DOC-2FDOC-3A-40LOCATIVE-TYPES-2040ANTS-DOC-2FLOCATIVES-3ASECTION-29).

The transcript in the code block tagged with `cl-transcript` is
automatically checked for up-to-dateness. See
`40`ANTS-DOC`/`TRANSCRIBE::@TRANSCRIPT``.
...
start with the character `@`. See `40`ANTS-DOC`/`DOC::@TUTORIAL`` for an example.
...
[`SECTION`](#x-2840ANTS-DOC-2FLOCATIVES-3ASECTION-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29))` says that ``@BAR`` is a subsection of this
...
arguments. See 40`ANTS-DOC`/`DOC:@LOCATIVE-TYPES` for the list of locative
...
The references are not looked up (see 40`ANTS-DOC`/`REFERENCE::RESOLVE` in the
40`ANTS-DOC`/`DOC:@EXTENSION-API`) until documentation is generated, so it is
...
`(`OBJECT` [`LOCATIVE`](#x-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29))` pair or `NIL`, to which the heading will link when
...
not have anything like that, instead it relies on 40`ANTS-DOC`/`REFERENCE::REFERENCE` objects
...
See [`DESCRIBE-OBJECT`](#x-28DESCRIBE-OBJECT-20-28METHOD-20NIL-20-2840ANTS-DOC-3ASECTION-20T-29-29-29) `([`METHOD`](#x-28METHOD-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29) () ([`SECTION`](#x-2840ANTS-DOC-2FLOCATIVES-3ASECTION-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29) T))`.

There are quite a few special variables that affect how output is
generated, see 40`ANTS-DOC`/`DOC:@DOCUMENTATION-PRINTER-VARIABLES`.
...
40`ANTS-DOC`/`REFERENCE-API::COLLECT-REACHABLE-OBJECTS`.
...
Finally, :`SOURCE-URI-FN` is a function of a single, 40`ANTS-DOC`/`REFERENCE::REFERENCE`
argument. If it returns a value other than `NIL`, then it must be a
string representing an URI. If FORMAT is :HTML and
40`ANTS-DOC`/`BUILDER`/`VARS`::`DOCUMENT-MARK-UP-SIGNATURES` is true, then the locative as
displayed in the signature will be a link to this uri. See
40`ANTS-DOC`/`GITHUB::MAKE-GITHUB-SOURCE-URI-FN`.

:PAGES may look something like this:

```commonlisp
`((;; The section about [`SECTION`](#x-2840ANTS-DOC-2FLOCATIVES-3ASECTION-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29)s and everything below it ...
...
#### [function] `UPDATE-ASDF-SYSTEM-HTML-DOCS` SECTIONS ASDF-SYSTEM &KEY PAGES (TARGET-DIR (ASDF/SYSTEM:SYSTEM-RELATIVE-PATHNAME 40ANTS-DOC/BUILDER::ASDF-SYSTEM "doc/")) (UPDATE-CSS-P T)
...
40`ANTS-DOC`/`BUILDER`/`VARS`::`DOCUMENT-MAX-NUMBERING-LEVEL` in dynamic HTML table of contents on
...
40`ANTS-DOC`/`BUILDER::UPDATE-ASDF-SYSTEM-README`) so that users have something to read
...
idea to add section like the 40`ANTS-DOC`/`DOC:@LINKS` section to allow jumping
...
the :`PAGES` argument of 40`ANTS-DOC`/`DOCUMENT::DOCUMENT`). The function looks the source
...
Such feature is not implemented in the 40`ANTS-DOC` system yet, but probably it will be
...
40`ANTS-DOC` normalizes indentation by converting:
...
See `40ANTS-DOC/DOCUMENT::DOCUMENT-OBJECT` ([`1`](#x-2840ANTS-DOC-2FDOCUMENT-3A-3ADOCUMENT-OBJECT-20-28METHOD-20NIL-20-2840ANTS-DOC-2FREFERENCE-3A-3AREFERENCE-20T-29-29-29) [`2`](#x-2840ANTS-DOC-2FDOCUMENT-3A-3ADOCUMENT-OBJECT-20-28METHOD-20NIL-20-28STRING-20T-29-29-29) [`3`](#x-2840ANTS-DOC-2FDOCUMENT-3A-3ADOCUMENT-OBJECT-20GENERIC-FUNCTION-29)) for the details.
...
from 40`ANTS-DOC` and you are set. The language tag, `elisp` in this example,
is optional and defaults to `common-lisp`.

See the documentation of [3`BMD`](https://github.com/3b/3bmd) and [colorize](https://github.com/redline6561/colorize/) for
the details.
...
combined with 40`ANTS-DOC`/`LINK`::`DOCUMENT-LINK-CODE`. For example, this docstring:
...
```text
"\\40`ANTS-DOC` *\\`DOCUMENT-NORMALIZE-PACKAGES*`"
```
...
40`ANTS-DOC`/`LOCATIVES:SECTION` locative). Back in the example above, clearly,
...
40`ANTS-DOC`/`BUILDER`/`PRINTER`::`DOCUMENT-UPPERCASE-IS-CODE` to have links generated for
...
links to sources (if :`SOURCE-URI-FN` is provided, see 40`ANTS-DOC`/`DOCUMENT::DOCUMENT`), and
...
table of contents off. If 40`ANTS-DOC`/`LINK`::`DOCUMENT-LINK-SECTIONS` is true, then the
...
40`ANTS-DOC`/`LINK`::`DOCUMENT-LINK-SECTIONS` to be on to work.
...
`FOO`
```
will be linked to (if 40`ANTS-DOC`/`LINK`::`DOCUMENT-LINK-CODE`) its definition. However,

```text
[`FOO`][dislocated]
```
will not be. On a dislocated locative 40`ANTS-DOC`/`LOCATIVES`/`BASE::LOCATE` always fails with a
...
An alias for 40`ANTS-DOC`/`LOCATIVES::DISLOCATED`, so the one can refer to an argument of a
...
`[`VARIABLE`](#x-28VARIABLE-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29)` in `([`VARIABLE`](#x-28VARIABLE-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29) LOCATIVE)`, this is what makes it possible
to land at the `(`DEFINE-LOCATIVE-TYPE` [`VARIABLE`](#x-28VARIABLE-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29) ...)` form.
...
When DISCARD-DOCUMENTATION-P (defaults to 40`ANTS-DOC`::DISCARD-DOCUMENTATION-P)
...
Refers to a glossary term defined by 40`ANTS-DOC`/`GLOSSARY::DEFINE-GLOSSARY-TERM`.
...
FOO)`. `(`MAKE-REFERENCE` 'FOO '[`VARIABLE`](#x-28VARIABLE-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29))` constructs a 40`ANTS-DOC`/`REFERENCE::REFERENCE` that
...
which we will see in 40`ANTS-DOC`/`LOCATIVES`/`BASE::LOCATE-AND-COLLECT-REACHABLE-OBJECTS`,
40`ANTS-DOC`/`LOCATIVES`/`BASE::LOCATE-AND-DOCUMENT` and 40`ANTS-DOC`/`LOCATIVES`/`BASE::LOCATE-AND-FIND-SOURCE`.

Naturally, `(`LOCATE` 'FOO '[`FUNCTION`](#x-28FUNCTION-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29))` will simply return `#'FOO`, no
...
40`ANTS-DOC`/`REFERENCE::REFERENCE` if there is no first class object corresponding to the
...
A convenience function to 40`ANTS-DOC`/`LOCATIVES`/`BASE::LOCATE` REFERENCE's object with its
locative.
...
One may wish to make the 40`ANTS-DOC`/`DOCUMENT::DOCUMENT` function and `M-.` navigation
work with new object types. Extending 40`ANTS-DOC`/`DOCUMENT::DOCUMENT` can be done by
defining a 40`ANTS-DOC`/`DOCUMENT::DOCUMENT-OBJECT` method. To allow these objects to be
referenced from [`DEFSECTION`](#x-2840ANTS-DOC-3ADEFSECTION-20-2840ANTS-DOC-2FLOCATIVES-3AMACRO-29-29), a 40`ANTS-DOC`/`LOCATIVES`/`BASE::LOCATE-OBJECT` method is to be defined.
Finally, for `M-.` 40`ANTS-DOC`/`SOURCE-API::FIND-SOURCE` can be specialized. Finally,
...
being able to reference `(LOCATIVE-TYPE [`LOCATIVE`](#x-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29))`. For example, if
you have:
...
(define-locative-type variable (&optional initform)
  "Dummy docstring.")
```
then `([`VARIABLE`](#x-28VARIABLE-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29) [`LOCATIVE`](#x-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29))` refers to this form.
...
refer. For example, if LOCATIVE-TYPE is the symbol [`PACKAGE`](#x-28PACKAGE-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29), this
...
40`ANTS-DOC`/`REFERENCE::REFERENCE` is returned then it must be canonical in the sense that
calling 40`ANTS-DOC`/`REFERENCE-API::CANONICAL-REFERENCE` on it will return the same reference.
...
Return a 40`ANTS-DOC`/`REFERENCE::REFERENCE` that resolves to OBJECT.
...
that would be documented in a (40`ANTS-DOC`/`DOCUMENT::DOCUMENT` OBJECT) call.
...
This function is called by 40`ANTS-DOC`/`SWANK::LOCATE-DEFINITION-FOR-EMACS` which lies
behind the `M-.` extension (see 40`ANTS-DOC`/`DOC:@EMACS-INTEGRATION`).
...
Let's see how to extend 40`ANTS-DOC`/`DOCUMENT::DOCUMENT` and `M-.` navigation if there is
no first class object to represent the thing of interest. Recall
that 40`ANTS-DOC`/`LOCATIVES`/`BASE::LOCATE` returns a 40`ANTS-DOC`/`REFERENCE::REFERENCE` object in this case. 40`ANTS-DOC`/`DOCUMENT::DOCUMENT-OBJECT`
and 40`ANTS-DOC`/`SOURCE-API::FIND-SOURCE` defer to 40`ANTS-DOC`/`LOCATIVES`/`BASE::LOCATE-AND-DOCUMENT` and
40`ANTS-DOC`/`LOCATIVES`/`BASE::LOCATE-AND-FIND-SOURCE`, which have 40`ANTS-DOC`/`LOCATIVES`/`BASE::LOCATIVE-TYPE` in their argument
...
40`ANTS-DOC`/`LOCATIVES`/`BASE::LOCATE-AND-COLLECT-REACHABLE-OBJECTS` on the object, locative-type,
...
Called by 40`ANTS-DOC`/`REFERENCE-API::COLLECT-REACHABLE-OBJECTS` on
40`ANTS-DOC`/`REFERENCE::REFERENCE` objects, this function has essentially the same purpose as its
...
If REFERENCE can be resolved to a non-reference, call
40`ANTS-DOC`/`DOCUMENT::DOCUMENT-OBJECT` with it, else call 40`ANTS-DOC`/`LOCATIVES`/`BASE::LOCATE-AND-DOCUMENT` on the
...
Called by 40`ANTS-DOC`/`DOCUMENT::DOCUMENT-OBJECT` on 40`ANTS-DOC`/`REFERENCE::REFERENCE` objects,
this function has essentially the same purpose as 40`ANTS-DOC`/`DOCUMENT::DOCUMENT-OBJECT`
...
If REFERENCE can be resolved to a non-reference, call 40`ANTS-DOC`/`SOURCE-API::FIND-SOURCE`
with it, else call 40`ANTS-DOC`/`LOCATIVES`/`BASE::LOCATE-AND-FIND-SOURCE` on the object,
...
Called by 40`ANTS-DOC`/`SOURCE-API::FIND-SOURCE` on 40`ANTS-DOC`/`REFERENCE::REFERENCE` objects, this
function has essentially the same purpose as 40`ANTS-DOC`/`SOURCE-API::FIND-SOURCE` generic-function but it has
...
This default implementation simply calls 40`ANTS-DOC`/`SOURCE-API::FIND-SOURCE` with OBJECT
...
40`ANTS-DOC`/`LOCATIVES`/`DEFINERS::DEFINE-SYMBOL-LOCATIVE-TYPE` and
40`ANTS-DOC`/`LOCATIVES`/`DEFINE-DEFINER::DEFINE-DEFINER-FOR-SYMBOL-LOCATIVE-TYPE` macros work together to
...
Similar to 40`ANTS-DOC`/`LOCATIVES`/`BASE::DEFINE-LOCATIVE-TYPE` but it assumes that all things
locatable with LOCATIVE-TYPE are going to be just symbols defined
with a definer defined with 40`ANTS-DOC`/`LOCATIVES`/`DEFINE-DEFINER::DEFINE-DEFINER-FOR-SYMBOL-LOCATIVE-TYPE`.
...
(define-symbol-locative-type direction ()
  "A direction is a symbol. (After this `M-.` on ``DIRECTION` [`LOCATIVE`](#x-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29)`
                                   works and it can also be included in [`DEFSECTION`](#x-2840ANTS-DOC-3ADEFSECTION-20-2840ANTS-DOC-2FLOCATIVES-3AMACRO-29-29) forms.)")
...
with 40`ANTS-DOC`/`LOCATIVES`/`DEFINERS::DEFINE-SYMBOL-LOCATIVE-TYPE`.
...
[`40ANTS-DOC::DEFSECTION`](#x-2840ANTS-DOC-3ADEFSECTION-20-2840ANTS-DOC-2FLOCATIVES-3AMACRO-29-29) and `40`ANTS-DOC`/`DOCUMENT::DOCUMENT`` cover most needs. However, it is plausible
...
[`DEFSECTION`](#x-2840ANTS-DOC-3ADEFSECTION-20-2840ANTS-DOC-2FLOCATIVES-3AMACRO-29-29) stores its :`NAME`, :`TITLE`, :[`PACKAGE`](#x-28PACKAGE-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29),
:`READTABLE` and :`ENTRIES` in SECTION objects.
...
this [`SECTION`](#x-2840ANTS-DOC-2FLOCATIVES-3ASECTION-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29) object.
...
[`PACKAGE`](#x-28PACKAGE-20-2840ANTS-DOC-2FLOCATIVES-3ALOCATIVE-29-29) will be bound to this package when
...
A 40`ANTS-DOC`/`REFERENCE::REFERENCE` or `NIL`. Used in generated documentation.
...
A list of strings and 40`ANTS-DOC`/`REFERENCE::REFERENCE` objects in the
...
[`40ANTS-DOC:SECTION`](#x-2840ANTS-DOC-3ASECTION-20CLASS-29) objects are printed by calling 40`ANTS-DOC`/`DOCUMENT::DOCUMENT` on them
with all 40`ANTS-DOC`/`DOC:@DOCUMENTATION-PRINTER-VARIABLES`, except for
`40`ANTS-DOC`/`BUILDER`/`PRINTER::*DOCUMENT-NORMALIZE-PACKAGES*``, turned off to reduce clutter.
...
^
```
then invoke the elisp function `40`ANTS-DOC-RETRANSCRIBE-REGION`` to get:
...
##### [function] `TRANSCRIBE` INPUT OUTPUT &KEY UPDATE-ONLY (INCLUDE-NO-OUTPUT 40ANTS-DOC/TRANSCRIBE::UPDATE-ONLY) (INCLUDE-NO-VALUE 40ANTS-DOC/TRANSCRIBE::UPDATE-ONLY) (ECHO T) CHECK-CONSISTENCY DEFAULT-SYNTAX (INPUT-SYNTAXES 40ANTS-DOC/TRANSCRIBE:\*SYNTAXES\*) (OUTPUT-SYNTAXES 40ANTS-DOC/TRANSCRIBE:\*SYNTAXES\*)
